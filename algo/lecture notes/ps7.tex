...
\begin{itemize}[<+->]
	\item \E{$(1,1,1)*(2,0,1)=$}
	\item \E{$(x^2+x+1)\cdot (x^2+2) =x^4+x^3+3x^2+2x+2= (2,2,3,1,1)$}
	\item מכפלת הערכים:
	\begin{tabular}{ |c|c|c|c| } 
		\hline
		x & $p_1(x)$ & $p_2(x)$ & $p_1(x)\cdot p_2(x)$ \\ 
		\hline
		0 & 1 & 2 & 2 \\ 
		1 & 3 & 3 & 9 \\ 
		2 & 7 & 6 & 42 \\ 
...
\pause
\begin{itemize}[<+->]
	\item \textbf{מעבר מייצוג מקדמים לייצוג בנקודות} נתון הפולינום \E{$(3,1,2)$} בייצוג מקדמים. חשבו אותו 
	\item \textbf{מעבר מייצוג בנקודות לייצוג מקדמים} חשבו את מקדמי הפולינום אשר יצוג הערכים שלו באותם בנקודות \E{$(0,1,2)$} הוא \E{$(1,3,7)$}
\end{itemize}
\pause נבחין שזמן הריצה של כל אחד מהמעברים האלה הוא \E{$\Theta(n^2)$}
\end{frame}
...
\pause
\begin{itemize}[<+->]
	\item באמצעות שיטת הפרד ומשול
	\item פשוט נחלק באמצע האם זה יעבוד?
