...
\begin{frame}{דוגמא 1}
\textbf{הגדרה}
יהי  \E{$G=(V,E)$} גרף לא מכוון ויהיו \E{$u,v\in V$} זוג קודקודים ב- \E{$G$}.  \pause נסמן ב-  \E{$\delta(u,v)$} את אורך המסלול המינימאלי בצלעות מקודקוד \E{$u$} לקודקוד \E{$v$}.  \pause בנוסף, נגדיר כי \E{$\delta(u,v)=\infty$}   אם לא קיים מסלול מ-u ל- v ב- G.   \pause 

\textbf{הגדרת בעיית \E{SP (Single Source Shortest Paths)}:}\\ \pause
יהי  \E{$G=(V,E)$} גרף לא מכוון ו- \E{$s\in V$} קודקוד ב- \E{$G$}. \pause מצא \E{$\delta(s,v) $} לכל 
\E{$v\in V$}. 
...
\LR{
	\fbox{
		\parbox{8cm}{
			{\sc $t=0\quad$ color[$u$]=white for all $u\in V$\\ } 
			{\sc DFS($u$) } 
			\begin{itemize}
				\item[] color[$u$] = gray
				\item[] $d[u]$ = t
