...
%אני אורן
% למדתי ולימדתי בעבר בבן גוריון וכעת זו הפעם הראשונה שאני מלמד באו"פ
...
% הצגה של התלמידים
% לשאול מי לא עשה את הקורס של מבוא לאלגוריתמים.
%הזמנה של יוחאי ולאה לכיתה בב"ש
...
\end{itemize}
\end{frame}
\begin{frame}{גרפים - תרגיל}
נתון גרף מכוון $G$, השלם את זמני הריצה בטבלה:
\begin{center}
	\begin{tabular}{ |c|c|c|c| } 
		\hline
		מטריצה & רשימה & שאלה \\
		\hline
	 \onslide<3->{$O(|V|)$} &  \onslide<2->{$O(1)$} & $(u,v)\in E$? \\ 
		\onslide<5->{$O(|V|)$} & \onslide<4->{$O(|V|^2)$} & האם הגרף ריק? \\ 
\onslide<7->{$O(|V|)$}	& \onslide<6->{$\Theta(|V|)$} & 	מצא את כל שכניו של צומת $v$   כלשהיא \\ 
		\hline
	\end{tabular}
\end{center}
\end{frame}
